...
/** Clear the circuit. Call once at the top of setup(). */
function bbReset() {
  bbParts = [];
  bbNets = {};
  bbGroundNet = null;
...
/** Current through a part in milliamps (0 if it is not conducting). */
...
/** Small label drawn just above a component body, upright regardless of rotation. */
function bbDrawLabel(text2, x, y, col) {
  if (!text2) return;
  push();
  noStroke();
  fill(col || 'black');
  textAlign(CENTER, BOTTOM);
  textSize(max(9, BB.pitch * 0.72));
  text(text2, x, y);
  pop();
}
...
function bbButton(o) {
  ...
  const part = {
    ...
    render: function () {
      ...
      // Cap - lighter while held down, which reads as "pressed in"
      noStroke();
      fill(this.pressed ? 'lightgray' : this.color);
...
function bbUpdateNonlinearStates(branches, V, openBranches) {
  ...
  // A transistor turns on once its base-emitter junction is forward biased.
  for (const part of bbParts) {
    if (part.type !== 'npn') continue;
    const vb = V[part.base.net] === undefined ? 0 : V[part.base.net];
    const ve = V[part.emitter.net] === undefined ? 0 : V[part.emitter.net];
    const on = (vb - ve) > 0.65;
...
function bbDrawScope(x, y, w, h, title) {
  ...
  for (const t of bbScopeTraces) {
    ...
    for (let i = 0; i < t.data.length; i++) {
      const px = map(i, 0, BB_SCOPE_POINTS - 1, plotX, plotX + plotW);
  ...
  if (bbScopeTraces.length) {
    textSize(12);
    let legendW = 0;
    for (const t of bbScopeTraces) {
      legendW = max(legendW, textWidth(t.label + ': 000.0 ' + t.unit) + 26);
    }
    noStroke();
    fill(255, 255, 255, 225);
    rect(plotX + 4, plotY + 3, min(legendW, plotW - 8), bbScopeTraces.length * 17 + 5, 3);
    textAlign(LEFT, CENTER);
    let ly = plotY + 14;
    for (const t of bbScopeTraces) {
      ...
      fill('black');
      text(t.label + ': ' + nf(last, 0, 1) + ' ' + t.unit, plotX + 28, ly);
      ly += 17;
    }
  }
  pop();
